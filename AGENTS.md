...
- **设置面板同步卡片重排（v0.5.1，v0.6.0 再排）**：连接状态改成小圆点 + 文字，**移动端只显示圆点**（`.app-shell.mobile .conn-text{display:none}`）。v0.6.0 的卡片结构：**通信方式下拉**（局域网/自建服务；P2P 未上线不显示，不给点了没反应的选项）→ 模式专有块（局域网：本机作为服务器勾选 + 主机名字输入 + `.sync-host` 运行状态行 + 管理台行；或「局域网主机 + 已选定 + 发现」三列一行 `.sync-peer-row`；自建服务：地址输入）→ 配对信息或账户表单（统一「开始同步」按钮，`.sync-actions.single` 占满一行）→ 三勾选 + 两间隔 + 三按钮。v0.5.1 的「地址输入框内嵌放大镜」已删除（发现改在局域网主机行）。按钮样式仍只许 `settings-button` / `menu-action-button` 两类。
...
- **传输分层（v0.6.0）**：`sync/transport.rs`（HTTP 客户端 12 个方法，三种方式共用一份实现）/ `sync/endpoint.rs`（「这一轮连哪儿」：自建服务直连、局域网按名字找主机、P2P 留位报 `SYNC_MODE_UNSUPPORTED`）/ `engine.rs`（pull→merge→push 编排）/ `merge.rs`+`crypto.rs`（不变）。**换通信方式只动 endpoint，不动内核**——这是分层的全部意义。`sync.mode` = `lan|server|p2p`；缺省时按 `serverUrl` 是否填过推断（`SyncSettings::effective_mode` 与前端 `normalizeSyncMode` 同口径），所以 v0.5.1 的配置升上来直接可用，不会掉进「还没选主机」的空状态。
...
- **P2P 调研结论（v0.6.1 计划，勿重复调研）**：croc 否决——角色构造时定死、单向一次性（双向要跑两次会话）、无机器接口（无 JSON/无超时/非 Windows 上 `send --code` 静默 exit 0）、Android 不能 exec、v11 硬拒 v10 peer。选型 **iroh 1.1**（纯 Rust、`SecretKey::from_bytes` 确定性身份、QUIC+打洞+n0 公共 relay）+ **账户派生的 pkarr 目录**：`iroh_dns::pkarr::SignedPacket::from_txt_strings` 用账户密钥签一条 TXT（记录 ≤1000 字节），`PkarrRelayClient` 走 `https://dns.iroh.link/pkarr` 发布/解析，无需第二个 Endpoint，实现「同账户密码互相发现」。硬约束：iroh relay 无 store-and-forward，**只有两台设备同时在线才同步**；n0 公共 relay 官方口径只适合 dev/testing；MSRV 1.91 / edition 2024 会抬高整个 workspace。
