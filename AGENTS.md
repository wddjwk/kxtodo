...
- **实体与范围**：node（含背景）/task/schedule(仅 spec+enabled)/settings(仅共享子集：profile/配色/特性开关；lifecycle/shortcuts/字号/sync 配置本身均为本机偏好)。scope 默认 data 开、settings/schedules 关；**sync configure 开启 scope 会重置拉取水位全量重拉**（增量流曾按旧 scope 过滤）。
- **数据地基（同步前置，schema v6）**：ID 128-bit 随机 hex（32-bit 会跨设备碰撞）；Node/Item 显式 `order: f64` 字段（同级排序唯一来源，数组顺序仅是渲染缓存，`gui.apply-tree-order` 写 order 不再裸排数组）；`collapsed`/`expanded` 是本机 UI 状态不参与同步。同步状态在 `runtime/sync.json`（deviceId/token/拉取水位/逐实体对账戳，0600），不属于三个 domain 文件。
...
- **统一 pair（v0.6.0）**：`sync register` / `sync login` 合并成 `sync pair`（前端一个「开始同步」按钮，账户不存在当场创建）；`endpoint::PairRequest` 描述「往哪儿配」；配对只写当前模式对应的地址字段（局域网配对不抹掉自建服务地址，反之亦然），所以切换方式不用重填另一种。CLI 路由注意：数据目录有常驻 Host 时 CLI 的**所有**命令都经 IPC 转给 Host 执行，所以 `sync configure --lan-host true` 会真的让正在运行的 GUI 起停内置服务器。
