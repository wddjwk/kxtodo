...
- `src-tauri/src/main.rs` owns local JSON persistence, export file writing, window-state support, native WebView zoom, single-instance activation, tray lifecycle, autostart control, the global toggle shortcut, and system-browser link opening.
...
- `Settings` stores editable profile information (including uploaded avatar data URLs), display preferences (native WebView UI scale and link-opening mode), lifecycle preferences (close-to-tray and autostart), local shortcut bindings, the global toggle shortcut, and disabled cloud-sync configuration.
...
4. Package a release with `.\scripts\package.ps1 -Version 4.1.0`.
