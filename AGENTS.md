...
- **配对流程**：`sync register`（新账户=注册+配对+首同步，全新设备先播种默认数据）→ 其它设备 `sync login`（全新设备首拉直接落服务端内容，**不与本地默认数据并集**）→ `sync now` / `sync status` / `sync configure` / `sync unpair`（清 token 关同步，服务器数据保留）。同步互斥锁 `runtime/sync.lock`（CLI/Host 并发只有一个同步在跑）。
...
- **配对历史（v0.5.1）**：`runtime/sync-history.json`（0600，MRU 最多 8 条：serverUrl/username/secret/usedAt），`pair_device` 登录成功后记录；设置页「数据同步」标题右侧历史图标按钮打开浮层一键回填（密码默认打码带显隐切换、可单条删除）；CLI `sync history [--remove N]`。密码明文落盘与 `settings.json` 的 `sync.secret` 同等级，不额外降低安全等级。
- **设置面板同步卡片重排（v0.5.1）**：连接状态改成小圆点 + 文字，**移动端只显示圆点**（`.app-shell.mobile .conn-text{display:none}`）——原来「服务器（🔴 已掉线）」整串和地址挤在同一行 grid 里会重叠；服务器地址单独一行 `word-break:break-all`；「发现」按钮换成服务器地址输入框内右侧的放大镜图标按钮，结果与配对历史共用 `.sync-popover` 下拉浮层（不再内联撑开卡片）。
