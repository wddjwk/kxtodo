...
//! 自定义类型不在表里时，标签回落到 kind 字符串本身、颜色回落到灰——界面上不开天窗。
...
export function accountTypeIcon(kind: string): string {
  return preset(kind)?.icon || ACCOUNT_KIND_ICON[kind] || "Wallet";
}

export function accountTypeColor(kind: string): string {
  return preset(kind)?.color || ACCOUNT_KIND_COLOR[kind] || "#7f8c8d";
}
