...
fn fresh_dir_has_default_inbox() {
    ...
    assert!(roots.iter().any(|node| node["kind"] == Value::Null || node["type"] == "entry"));
...
fn add_item_requires_existing_entry() {
    let env = TestEnv::fresh();
    let error = env.err(
        &["task", "add", "--type", "item", "--entry-id", "entry-nope", "--markdown", "x"],
        3,
    );
...
fn add_item_rejects_category_parent() {
    ...
    let error = env.err(
        &[
            "task", "add", "--type", "item",
            "--entry-id", category["id"].as_str().unwrap(),
            "--markdown", "x",
        ],
        2,
    );
...
fn list_and_filters() {
    ...
    env.ok(&["task", "add", "--type", "item", "--entry-id", &entry_id, "--markdown", "任务A", "--important", "true"]);
    env.ok(&["task", "add", "--type", "item", "--entry-id", &entry_id, "--markdown", "任务B", "--completed", "true"]);
    env.ok(&["task", "add", "--type", "item", "--entry-id", &entry_id, "--markdown", "任务C", "--due-date", "2026-07-30"]);

    let all = env.ok(&["task", "list", "--type", "item", "--entry-id", &entry_id, "--status", "all"]);
    assert_eq!(all["items"].as_array().unwrap().len(), 3);

    let open = env.ok(&["task", "list", "--type", "item", "--entry-id", &entry_id, "--status", "open"]);
    assert_eq!(open["items"].as_array().unwrap().len(), 2);

    let completed = env.ok(&["task", "list", "--type", "item", "--entry-id", &entry_id, "--status", "completed"]);
    assert_eq!(completed["items"].as_array().unwrap().len(), 1);

    let important = env.ok(&["task", "list", "--type", "item", "--entry-id", &entry_id, "--important", "true"]);
    assert_eq!(important["items"].as_array().unwrap().len(), 1);

    let due = env.ok(&[
        "task", "list", "--type", "item", "--entry-id", &entry_id,
        "--due-from", "2026-07-01", "--due-to", "2026-07-31",
    ]);
    ...
    let changed = env.ok(&[
        "task", "list", "--type", "item",
        "--changed-from", "2020-01-01", "--changed-to", "2100-01-01", "--all",
    ]);
    assert!(changed["items"].as_array().unwrap().len() >= 3);

    // 空列表是成功
    let empty = env.ok(&["task", "list", "--type", "item", "--entry-id", &entry_id, "--important", "false", "--status", "completed"]);
    let _ = empty;
}
...
fn list_pagination() {
    ...
    for index in 0..7 {
        env.ok(&[
            "task", "add", "--type", "item", "--entry-id", &entry_id,
            "--markdown", &format!("任务{index}"),
        ]);
    }
    let page1 = env.run(&["task", "list", "--type", "item", "--entry-id", &entry_id, "--limit", "3", "--sort", "createdAt", "--order", "asc"]);
    ...
    let page2 = env.ok(&[
        "task", "list", "--type", "item", "--entry-id", &entry_id,
        "--limit", "3", "--cursor", &cursor, "--sort", "createdAt", "--order", "asc",
    ]);
    assert_eq!(page2["items"].as_array().unwrap().len(), 3);
    let all = env.ok(&["task", "list", "--type", "item", "--entry-id", &entry_id, "--all"]);
...
fn find_searches_markdown_names_and_tags() {
    ...
    env.ok(&[
        "task", "add", "--type", "item", "--entry-id", &entry_id,
        "--markdown", "完成 XXX 需求", "--tag", "blue:需求",
    ]);

    let by_markdown = env.ok(&["task", "find", "--query", "XXX"]);
    assert!(by_markdown["items"].as_array().unwrap().iter().any(|item| item["type"] == "item"));
...
fn modify_item_state_dates_and_move() {
    ...
    let item = env.ok(&["task", "add", "--type", "item", "--entry-id", &entry_id, "--markdown", "任务"]);
    let item_id = item["id"].as_str().unwrap().to_string();
    let created_updated = item["updatedAt"].as_str().unwrap().to_string();

    std::thread::sleep(std::time::Duration::from_millis(20));
    let done = env.ok(&["task", "modify", "--type", "item", "--id", &item_id, "--completed", "true"]);
    assert_eq!(done["completed"], true);
    assert!(done["completedAt"].is_string());
    assert_ne!(done["updatedAt"].as_str().unwrap(), created_updated);

    let reopened = env.ok(&["task", "modify", "--type", "item", "--id", &item_id, "--completed", "false"]);
    assert_eq!(reopened["completed"], false);
    assert!(reopened["completedAt"].is_null());

    let dated = env.ok(&[
        "task", "modify", "--type", "item", "--id", &item_id,
        "--planned-date", "2026-07-28", "--due-date", "2026-07-31",
    ]);
    assert_eq!(dated["plannedDate"], "2026-07-28");
    let cleared = env.ok(&[
        "task", "modify", "--type", "item", "--id", &item_id, "--clear-planned-date",
    ]);
    assert!(cleared["plannedDate"].is_null());

    let moved = env.ok(&["task", "modify", "--type", "item", "--id", &item_id, "--entry-id", &entry2_id]);
...
fn modify_tags_and_emojis() {
    ...
    let item = env.ok(&[
        "task", "add", "--type", "item", "--entry-id", &entry_id,
        "--markdown", "x", "--tag", "blue:需求", "--emoji", "🔥",
    ]);
    let item_id = item["id"].as_str().unwrap().to_string();
    let tag_id = item["tags"][0]["id"].as_str().unwrap().to_string();

    let added = env.ok(&["task", "modify", "--type", "item", "--id", &item_id, "--add-tag", "red:紧急"]);
    assert_eq!(added["tags"].as_array().unwrap().len(), 2);

    let removed = env.ok(&["task", "modify", "--type", "item", "--id", &item_id, "--remove-tag", &tag_id]);
    assert_eq!(removed["tags"].as_array().unwrap().len(), 1);
    assert_eq!(removed["tags"][0]["text"], "紧急");

    let replaced = env.ok(&[
        "task", "modify", "--type", "item", "--id", &item_id,
        "--replace-tags", "green:a", "gray:b",
    ]);
    ...
    let error = env.err(&[
        "task", "modify", "--type", "item", "--id", &item_id, "--add-tag", "pink:无效",
    ], 2);
...
fn modify_nodes_and_move_validation() {
    ...
    let c2 = env.ok(&["task", "add", "--type", "category", "--name", "B", "--parent-id", &c1_id]);
    let c2_id = c2["id"].as_str().unwrap().to_string();
    let entry = env.ok(&["task", "add", "--type", "entry", "--name", "e"]);
    let entry_id = entry["id"].as_str().unwrap().to_string();

    let renamed = env.ok(&["task", "modify", "--type", "entry", "--id", &entry_id, "--name", "e2", "--icon", "star"]);
    assert_eq!(renamed["name"], "e2");

    let moved = env.ok(&["task", "modify", "--type", "entry", "--id", &entry_id, "--parent-id", &c2_id]);
    assert_eq!(moved["parentId"], c2_id);
    assert_eq!(moved["path"], "A / B / e2");

    // 分类循环必须被拒绝
    let error = env.err(&[
        "task", "modify", "--type", "category", "--id", &c1_id, "--parent-id", &c2_id,
    ], 4);
    assert_eq!(error["code"], "CATEGORY_CYCLE");

    // 节点不能放到 entry 下
    let error = env.err(&[
        "task", "modify", "--type", "category", "--id", &c2_id, "--parent-id", &entry_id,
    ], 2);
    assert_eq!(error["code"], "INVALID_PARENT");

    let collapsed = env.ok(&["task", "modify", "--type", "category", "--id", &c2_id, "--collapsed", "true"]);
...
fn remove_requires_confirmation_and_cascade() {
    let env = TestEnv::fresh();
    let (_c, entry_id) = setup_entry(&env);
    let item = env.ok(&["task", "add", "--type", "item", "--entry-id", &entry_id, "--markdown", "x"]);
    let item_id = item["id"].as_str().unwrap().to_string();
    ...
    let error = env.err(&["task", "remove", "--type", "entry", "--id", &entry_id, "--yes"], 4);
    assert_eq!(error["code"], "NOT_EMPTY");

    // dry-run 报告影响范围
    let dry = env.ok(&["task", "remove", "--type", "entry", "--id", &entry_id, "--cascade", "--dry-run"]);
    assert_eq!(dry["plan"]["totals"]["items"], 1);

    // 实际级联删除
    let removed = env.ok(&["task", "remove", "--type", "entry", "--id", &entry_id, "--cascade", "--yes"]);
    ...
    let confirmed = env.ok(&[
        "task", "add", "--type", "entry", "--name", "新条目", "--idempotency-key", "k1",
    ]);
...
fn system_nodes_are_readonly() {
    let env = TestEnv::fresh();
    env.err(&["task", "remove", "--type", "system", "--id", "my-day", "--yes"], 2);
    env.err(&["task", "modify", "--type", "system", "--id", "my-day", "--name", "x"], 2);
...
fn idempotency_replay_returns_first_resource() {
    let env = TestEnv::fresh();
    let first = env.run(&[
        "task", "add", "--type", "category", "--name", "幂等", "--idempotency-key", "same-key",
    ]);
...
fn if_revision_guard() {
    ...
    // 匹配的 revision 可以写入
    env.run(&[
        "task", "add", "--type", "category", "--name", "c2", "--if-revision", &revision.to_string(),
    ]);

    // 过期的 revision 冲突
    let error = env.err(&[
        "task", "add", "--type", "category", "--name", "c3", "--if-revision", &revision.to_string(),
    ], 4);
...
fn jq_filter_applies_to_success_envelope() {
    ...
    let entry = env.ok(&["task", "add", "--type", "entry", "--name", "e"]);
    let entry_id = entry["id"].as_str().unwrap().to_string();
    env.ok(&["task", "add", "--type", "item", "--entry-id", &entry_id, "--markdown", "a"]);
    env.ok(&["task", "add", "--type", "item", "--entry-id", &entry_id, "--markdown", "b"]);

    let result = env.run(&[
        "task", "list", "--type", "item", "--entry-id", &entry_id, "--jq", ".data.items | length",
    ]);
    assert_eq!(result.code, 0);
    assert_eq!(result.stdout.trim(), "2");

    let error = env.run(&[
        "task", "list", "--type", "item", "--entry-id", &entry_id, "--jq", ".bad(",
    ]);
...
fn unknown_fields_preserved_on_write() {
    ...
    // 正常修改
    env.ok(&["task", "modify", "--type", "entry", "--id", &entry_id, "--name", "e2"]);
...
fn markdown_file_and_stdin_inputs() {
    ...
    let item = env.ok(&[
        "task", "add", "--type", "item", "--entry-id", &entry_id,
        "--markdown-file", md_path.to_str().unwrap(),
    ]);
...
fn orphan_item_reported_as_integrity_issue() {
    ...
    let issues = list["integrityIssues"].as_array().expect("应报告 integrityIssues");
    ...
    let entry2_id = entry2["id"].as_str().unwrap().to_string();
    env.ok(&["task", "modify", "--type", "item", "--id", &item_id, "--entry-id", &entry2_id]);
    let list = env.ok(&["task", "list", "--type", "item", "--all"]);
