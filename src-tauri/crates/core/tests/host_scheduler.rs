...
use kxtodo_core::core::{ExecContext, HostServices, Invocation};
use kxtodo_core::error::CoreResult;
use kxtodo_core::host::{HostBackend, HostCore};
use kxtodo_core::ipc::{IpcClient, IpcControls, IpcRequest, PROTOCOL_VERSION};
...
fn execute_on(core: &Arc<HostCore>, command: &str, params: Value) -> Value {
    ...
    let outcome = kxtodo_core::core::execute(&invocation, &ctx);
    assert_eq!(
        outcome.code, 0,
        "{command} 执行失败：{}",
...
fn python_available() -> bool {
    !kxtodo_core::exec::runtime_path(&Default::default(), "python").is_empty()
}
...
fn ipc_roundtrip_executes_commands() {
    ...
    let endpoint = kxtodo_core::host::start_ipc_server(host.core.clone()).unwrap();
    ...
    let discovered = kxtodo_core::ipc::discover_host(&env.path());
...
fn scheduler_fires_once_notification() {
    let env = TestEnv::fresh();
    let host = start_headless(&env);
    let at = kxtodo_core::time::format_instant(
    ...
    assert_eq!(
        ...
        kxtodo_core::model::ScheduleStatus::Success
    );

    assert!(wait_for(5, || {
        kxtodo_core::history::read_history(&host.core.repo.layout.schedule_history())
...
fn scheduler_interval_script_stops_on_match() {
    ...
    assert_eq!(
        entry.state.last_status,
        kxtodo_core::model::ScheduleStatus::Stopped
    );
    assert_eq!(entry.state.run_count, 1);
...
fn probe_spawn_failure_does_not_increment_main_run_count() {
    ...
    assert_eq!(
        ...
        kxtodo_core::model::ScheduleStatus::Failed
    );
}
...
fn large_process_output_is_bounded_and_marked_truncated() {
    ...
    assert!(
        result["stdout"].as_str().unwrap().len()
            <= kxtodo_core::repo::SCHEDULE_OUTPUT_MAX_BYTES
    );
    assert!(
...
fn missed_once_runs_on_host_start() {
    ...
    let host = start_headless(&env);
    let past = kxtodo_core::time::format_instant(
...
fn stale_running_state_is_reconciled_on_start() {
    ...
    host.core
        ...
        .write_schedule_internal("test.seed-running", |file| {
            ...
            file.tasks[0].state.last_status = kxtodo_core::model::ScheduleStatus::Running;
            Ok(json!({ "id": id }))
        })
        .unwrap();
    host.core.start_scheduler();
