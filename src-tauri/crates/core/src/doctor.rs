...
use crate::model::{NodeKind, ScheduleSpec};
use crate::repo::{
...
pub fn config_paths(ctx: &ExecContext) -> CoreResult<Value> {
    ...
    let host_state = match crate::ipc::read_host_descriptor(&host) {
    ...
    Ok(json!({
        ...
        "limits": {
            "idempotencyPerDomain": crate::repo::IDEMPOTENCY_MAX_RECORDS,
            "idempotencyMaxAgeDays": crate::repo::IDEMPOTENCY_MAX_AGE_DAYS,
        ...
        "host": host_state,
        "skills": crate::skills::cmd_path().unwrap_or_default(),
    }))
}
...
pub fn run_doctor(inv: &Invocation, ctx: &ExecContext, _meta: &mut Meta) -> CoreResult<Value> {
    ...
    if selected == Some("runtimes") {
        ...
        match ctx.repo.load_schedule() {
            Ok(file) => {
                let view = crate::ops_schedule::runtime_view(&file);
                ...
                checks.push(check(
                    ...
                    if missing.is_empty() {
                        ...
                        Some("kxtodo-cli schedule runtime detect 或 runtime set <name> <path>")
                    },
                ));
            }
    ...
    for (name, path, expected) in [
        (
            "data",
            layout.data_file(),
            crate::model::DATA_SCHEMA_VERSION,
        ),
        (
            "settings",
    ...
    if let Ok(data) = ctx.repo.load_data() {
        let orphans = crate::ops_task::orphan_item_ids(&data);
        checks.push(check(
            "orphan-items",
            orphans.is_empty(),
            ...
            if orphans.is_empty() {
                ...
                Some("用 kxtodo-cli task modify --type item --id <id> --entry-id <entry> 修复")
            },
        ));
        if !orphans.is_empty() {
            checks.push(json!({
        ...
        let dangling: Vec<String> = data
            ...
            .filter(|node| {
                node.parent_id
                    .as_deref()
                    .map(|parent| {
                        crate::ops_task::find_node(&data, parent)
        ...
        if img_data.is_dir() {
            ...
            if let Ok(entries) = std::fs::read_dir(&img_data) {
                for entry in entries.flatten() {
                    ...
                    let referenced = crate::ops_task::find_node(&data, &name)
    ...
    if let Ok(file) = ctx.repo.load_schedule() {
        ...
        checks.push(check(
            ...
            if invalid.is_empty() {
                ...
                Some("kxtodo-cli schedule validate --id <id> --patch ...")
            },
        ));
    ...
    if !layout.lock_file().exists() {
        ...
        match crate::repo::RepoLock::try_acquire_existing(layout) {
    ...
    match crate::ipc::read_host_descriptor(&host_path) {
        Some(descriptor) => {
            let alive = crate::ipc::host_process_alive(descriptor.pid);
            checks.push(check(
                "host",
                true,
                format!(
    ...
    if let Ok(settings) = ctx.repo.load_settings() {
        let issues = crate::ops_config::validate_settings(layout, &settings);
        checks.push(check(
            "settings",
            issues.is_empty(),
