...
use crate::envelope::{failure, success, Meta};
use crate::error::{CoreError, CoreResult, ErrorKind};
use crate::model::NodeKind;
use crate::ops_config as config_ops;
use crate::ops_schedule as schedule_ops;
use crate::ops_task as task_ops;
...
pub trait HostServices: Send + Sync {
    ...
    fn apply_native_effect(
        &self,
        name: &str,
        settings: &crate::model::SettingsFile,
    ) -> CoreResult<()>;
...
fn cmd_notify(inv: &Invocation, ctx: &ExecContext, _meta: &mut Meta) -> CoreResult<Value> {
    ...
    if let Some(raw) = &duration {
        crate::time::parse_duration_ms(raw)?;
    }
...
fn schedule_validate(inv: &Invocation, ctx: &ExecContext, meta: &mut Meta) -> CoreResult<Value> {
    ...
    if let Some(spec_raw) = inv.params.get("spec") {
        ...
        crate::ops_schedule::normalize_spec_paths(&mut spec_json, &ctx.cwd);
...
fn schedule_logs(inv: &Invocation, ctx: &ExecContext, meta: &mut Meta) -> CoreResult<Value> {
    ...
    let history = crate::history::read_history(&ctx.repo.layout.schedule_history())?;
    ...
    match action {
        ...
        "set" => {
            ...
            let path = if raw_path.trim().is_empty() {
                ...
                let found = crate::exec::find_executable(&[raw_path.as_str()], &[]);
                if found.is_empty() {
...
fn load_settings_with_raw(
    ctx: &ExecContext,
) -> CoreResult<(crate::model::SettingsFile, Value)> {
    let settings = ctx.repo.load_settings()?;
    let raw = crate::repo::read_json_value(&ctx.repo.layout.settings_file())?;
    Ok((settings, raw))
}
