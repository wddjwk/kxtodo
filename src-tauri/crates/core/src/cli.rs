...
pub enum SyncAction {
    ...
    /// 列出 P2P 账户目录里的在线设备（Risk: read）
    #[command(
        long_about = "Risk: read\n\n解析本账户的 P2P 设备目录（账户派生密钥签名的 pkarr 记录），列出在线设备与本机：名字优先取\n对方发布的设备名记录（本机这条直接取本机正在发布的名字），没学到才退回拨号时从 /healthz\n学到的名字或 id 前缀，并附最近一次拨号结果。**同步一轮会跟列表里每台在线设备各对账一次**\n（含本机内置库）：谁发起谁就把数据推给对方再拉回来，不依赖选举；某台不在线本轮跳过它。\n_hubId 只是配对/连通性探测用的单端点回退口径（在线设备里 EndpointId 最小者）。设备名用\nsync configure --lan-name 设定（与局域网主机名同一个字段）。"
    )]
    Peers,
    /// 解除本机配对（Risk: write）
    #[command(
