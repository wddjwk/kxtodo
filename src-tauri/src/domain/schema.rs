...
use crate::domain::model::{Match, Notification, ScheduleSpec};
...
pub fn jq_schema() -> Value {
    serde_json::from_str(crate::domain::jq::JQ_SUBSET_DOC).unwrap_or(Value::Null)
}
    ...
    for segment in path.split('.') {
        let next = current
            ...
            .ok_or_else(|| {
                CoreError::not_found(
                    "SCHEMA_NOT_FOUND",
                    format!("未知命令 `{path}`；运行 kxtodo --help 查看命令树"),
                )
            })?;
