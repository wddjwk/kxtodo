...
use crate::domain::time::{now_iso, parse_duration_ms, parse_stored_instant};
...
pub fn validate_spec_value(raw: &Value, runtimes: &crate::domain::model::Runtimes) -> CoreResult<SpecValidation> {
    ...
    let spec: ScheduleSpec = serde_json::from_value(raw.clone()).map_err(|error| {
        CoreError::validation("INVALID_SPEC", format!("ScheduleSpec 校验失败：{error}"))
            .with_hint("运行 kxtodo schema schedule.spec 查看权威结构")
    })?;
    ...
    match &spec.action {
        Action::Script {
            ...
            let interpreter_set = interpreter
                ...
                .is_some();
            if !interpreter_set {
                let detected = crate::domain::exec::runtime_path(runtimes, language.runtime_key());
                if detected.is_empty() {
                    warnings.push(format!(
                        "未检测到 {} 运行时，执行前请先 kxtodo schedule runtime set {}",
                        language.as_str(),
...
pub fn match_stream(matcher: &crate::domain::model::Match, stdout: &str, stderr: &str) -> bool {
    let haystack = match matcher.stream {
        crate::domain::model::MatchStream::Stdout => stdout,
        crate::domain::model::MatchStream::Stderr => stderr,
    };
    match matcher.mode {
        crate::domain::model::MatchMode::Contains => haystack.contains(&matcher.pattern),
        crate::domain::model::MatchMode::Regex => Regex::new(&matcher.pattern)
            .map(|regex| regex.is_match(haystack))
            .unwrap_or(false),
    }
}

fn validate_notification(notification: &crate::domain::model::Notification) -> CoreResult<()> {
...
pub fn require_entry<'a>(file: &'a ScheduleFile, id: &str) -> CoreResult<&'a ScheduleEntry> {
    find_entry(file, id).ok_or_else(|| {
        CoreError::not_found("SCHEDULE_NOT_FOUND", format!("未找到定时任务 {id}"))
            .with_hint("先运行 kxtodo schedule list 或 schedule find --query ...")
    })
}
