...
fn planned_next(entry: &ScheduleEntry) -> (Option<String>, Option<Value>) {
    ...
    match crate::domain::plan::compute_next_run(entry, Utc::now()) {
...
struct FlowResult {
    result: CoreResult<crate::domain::exec::ExecOutput>,
...
fn cancelled_output() -> crate::domain::exec::ExecOutput {
    crate::domain::exec::ExecOutput {
        exit_code: None,
        stdout: String::new(),
...
fn execute_action_flow(
    ...
    if let Trigger::Condition { probe, when, .. } = &entry.spec.trigger {
        ...
        let probe_spec = match crate::domain::exec::build_probe_spec(probe, runtimes) {
            Ok(spec) => spec,
    ...
    let result = match &entry.spec.action {
        Action::Notification { notification } => core
            ...
            .map(|_| crate::domain::exec::ExecOutput {
                exit_code: Some(0),
                stdout: String::new(),
                stderr: String::new(),
                timed_out: false,
        ...
        _ => {
            crate::domain::exec::build_action_spec(&entry.spec.action, runtimes).and_then(|spec| {
