...
use crate::domain::ids::{gen_id, is_reserved_id};
use crate::domain::model::{DataFile, Item, Node, NodeKind, Tag, TagColor, SYSTEM_NODE_IDS};
use crate::domain::time::{now_iso, parse_date};
...
pub fn get_node_typed<'a>(
    ...
    let node = find_node(data, id).ok_or_else(|| {
        ...
        .with_hint("先运行 kxtodo task find 或 task tree 获取稳定 ID")
    })?;
...
fn ensure_entry_target(data: &DataFile, entry_id: &str) -> CoreResult<()> {
    let node = find_node(data, entry_id).ok_or_else(|| {
        CoreError::not_found("ENTRY_NOT_FOUND", format!("未找到条目 {entry_id}"))
            .with_hint("先运行 kxtodo task find --type entry --query ...")
    })?;
...
fn instant_in_range(value: Option<&str>, from_ms: Option<i64>, to_ms: Option<i64>) -> bool {
    ...
    let Ok(ms) = crate::domain::time::instant_millis(raw) else {
        return false;
    };
...
pub fn parse_range_bound(raw: &str, is_to: bool) -> CoreResult<i64> {
    ...
    crate::domain::time::instant_millis(value)
}
