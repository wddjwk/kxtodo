...
fn migrate_schedule_task(
    ...
    let last_run_at = raw
        ...
        .map(|at| crate::domain::time::format_instant(at));
    ...
    let trigger = match trigger_type.as_str() {
        ...
        "calendar" => {
            ...
            match (cron, tz) {
                (Some(cron), Some(zone)) => {
                    if let Err(error) = crate::domain::plan::validate_cron(&cron) {
                        trigger_ok = false;
    ...
    if serde_json::from_value::<crate::domain::model::ScheduleSpec>(spec.clone()).is_err() {
    ...
    if spec["enabled"].as_bool().unwrap_or(false) {
        ...
        match crate::domain::plan::compute_next_run_iso(&entry_stub, chrono::Utc::now()) {
            Ok(Some(next)) => {
...
fn migrate_action(raw: &Value, warnings: &mut Vec<String>) -> (Value, bool) {
    ...
    let args = match crate::domain::exec::split_legacy_arguments(legacy_args) {
        Ok(args) => args,
        Err(error) => {
