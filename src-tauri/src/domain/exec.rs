...
use crate::domain::model::{Action, Probe, Runtimes, ScriptLanguage, Source};
...
pub fn build_script_spec(
    ...
    if interpreter.is_empty() {
        return Err(CoreError::execution(
            "RUNTIME_NOT_FOUND",
            format!(
                "未找到 {} 的解释器，请运行 kxtodo schedule runtime detect 或 runtime set",
                language.as_str()
            ),
        ));
    }
    ...
    match action {
        ...
        Action::Script {
            ...
            *language,
            ...
            match timeout {
                Some(raw) => Some(crate::domain::time::parse_duration_ms(raw)?),
                None => None,
            },
            runtimes,
        )?)),
