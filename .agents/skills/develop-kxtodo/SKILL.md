...
3. **验证**——本地验证不了的平台（如无 WSL 时的 Linux、无真机时的 Android）交给 CI（ci.yml 双平台编译检查 / release.yml 三平台构建），但**必须明说"该端未验证"，不许默认没问题**。「交给 CI」只覆盖**编译与构建**；凡涉及**路径语义 / 平台 API / 交互时序**的改动，未验证清单必须**点名到「哪一端、哪条路径、由谁手测」**——清单写成免责声明而不是待办，就是「桌面选文件发送」那个 bug 跨三个版本存活的土壤（v0.8.6 评审 E1）。
...
- **探针先于定稿**（v0.8.6 起）：传输 / 路径语义 / 交互时序层的修复方案，**静态推演会被实测推翻**（曾有三个静态方案被探针一次性证伪）。先把探针或可跑的断言写出来（`scripts/v08X-fixes-test.mjs` 一开始就当探针用），再定实现。**断言要比对真实观感**——把手指落点差几像素、菜单底边与光标的关系、滚动条有无、面板宽度变不变——**只数事件次数或只看最终值的断言，实现歪了照样全绿**（色盘把手指歪 44px、取消不还原，v0.8.6 的全套断言都是绿的）。
...
- **点锚定的浮层几何只有一份（`popover.ts::placePopover`）**：优先向下、放不下翻到锚点上方且**下边缘对齐**、**四边一律钳制**。（**v0.8.7 终裁**：没有「下方限高 + 内部滚动」这一档，唯一滚动兜底是「上下都放不下」的 maxHeight 钳制；右键菜单翻上时 `mirrorXOnFlip` 镜像成右下角贴鼠标。）量尺寸要等**双 rAF**（首帧内容还没定宽，钳制会算错），落位前先 `visibility: hidden` 免得在错位置闪一帧。
...
- **新增 UI 先问「现有样式/组件/调用约定能不能复用」，不要自己造一套**（用户原话：「这些都是已有的逻辑，你要尽量复用，不要自己创造啊」）。反例：工具箱与工具子页的三点菜单最初把菜单开在鼠标位置上、还给取色各写一份状态——而现成的一整套是 `ContextMenu` + `MenuItem` + **点锚定约定**（`x = rect.right` / `y = rect.bottom + 6` / `xAlign="right"` / `anchor={按钮}`，菜单贴按钮正下方、右缘对齐按钮右缘）。**除非需求明确要求特殊样式，一律照抄同类的既有写法**——复用是验收项，不是建议。
- **浮层定位只有两条分支**：下方放得下 → 左上角贴锚点；放不下 → 整个翻上、下边缘贴锚点（`mirrorXOnFlip` 时右下角贴锚点）。**「下方限高 + 内部滚动」被真机打回，别再回来**。量高一律 `Math.max(rect.height / scale, scrollHeight)`——`scrollHeight` 本就是布局像素、**不能再除**（多除一次把菜单量高 33%，翻上后底边落不到锚点）。
- **缩放壳里挂第三方指针交互组件，让它「视觉尺寸 == 布局尺寸」**（iro 的拖动数学是「光标视觉坐标 ÷ 布局宽」，差一个缩放比就把手指按 `(1−uiScale)` 脱靶）。三件套：反缩放层（`transform: scale(1/--ui-scale)`、`origin: top left`）+ 宽度传**视觉口径**（宿主布局宽 × uiScale）+ 宿主高度显式补偿（`base.offsetHeight / uiScale`）；宽度比较用 `offsetWidth`（布局对布局）。**改了它必配几何断言**（把手指与光标差 < 3px）——只数事件次数测不出脱靶。
- **挂在 blur 上的提交 + 同步收尾 = 结构性丢改动**：确认按钮自己的 mousedown 就是输入框的 blur，而确认会把浮层 store 置 null、把飞行中的 rAF 作废（等再久也没用）。**确认前先冲刷 pending 草稿**（`flushDraftsIntoPicker` + `flushPendingPreview`），消费端同时改用**面板传回的实参**落盘。同类输入框（「即输即生效」的那些）一律停输 300ms 再落盘，且**菜单/弹层关闭时 flush 一次**。
- **配置键的作用域要对齐**：编辑端写什么键，消费端必须读同一个键。系统视图（我的一天/计划内/收藏/定时任务）里编辑端写**视图键**、卡片读**条目键**——`colorPreview.ts::dueColorsForCard` 双回退（自己条目优先、没配过才跟视图），预览走同一条链。
- **时间戳比较一律 `Date.parse`**：core 写入的 `…Z` 与 `…+08:00` 两种格式混存，按字面比较跨格式必错位；后果不只是排序难看——**封顶 200 会截掉真正最新的**。搜索排序与日记的天内排序同口径。
...
| 加浮层 / 弹层 / 全屏查看 | `platform.ts::createBackGuard`（**挂载式必须 `onDestroy(dispose)`**，见 3.8）+ 安全区避让 + **点锚定的几何走 `popover.ts::placePopover`**（下方放得下就左上角贴锚点、放不下整个翻上且下边缘贴锚点、`mirrorXOnFlip` 管右键菜单的镜像；量尺寸等双 rAF）；**按钮锚定的菜单照抄四件套**：`x = rect.right` / `y = rect.bottom + 6` / `xAlign="right"` / `anchor={按钮}`（同一个按钮再点一次收起）；卡片级「点别处关闭」的浮层挂进 `cardOverlays.ts` 的那**一份** document 监听 | 本文件 3.8 + 3.16 + `references/ui-patterns.md` |
...
| 改取色（主题色 / 背景色 / 临期色） | **12 处取色入口都是 `ColorPickerPanel.svelte` + `colorPickerPanel.ts` 的单例请求**（内嵌 iro，懒加载；RGB/HEX 双向联动 + 拖动按 rAF 合帧；**几何三件套见 3.16**——反缩放层 + 宽度传视觉口径 + 宿主高度补偿；**确认前必须 `flushDraftsIntoPicker` + `flushPendingPreview`**；**有 `EyeDropper` 的平台渲染吸管**；同入口重开作废旧会话 + key 加序号）；`src/lib/colorPreview.ts`（草稿预览 store + 三个纯选择器，含系统视图双回退 `dueColorsForCard`）+ 消费端 `Workspace`/`DiaryView`/`LedgerView`/`ToolboxView` 的 `mainStyle` 与 `TaskCard` 的临期配色；**落盘按钮只在色盘面板上**（宿主菜单里的 `ColorDraftActions` 已删）；预设色块单击即落盘、右键进「即输即生效」的编辑器（名称停输 300ms、颜色合法即写）；工具页外观分两层 `settings.toolbox.*`（主界面）与 `toolbox.toolAccents/toolBackgrounds`（每个工具），都进同步共享子集 | `references/frontend.md` + `references/history/v0.8.7.md` 二 |
| 加工具箱工具 / 把工具固定进侧栏 | **目录与注册表分开**：`src/lib/tools/catalog.ts`（id/名称/描述，侧栏也要认）与 `registry.ts`（图标 + 懒加载组件）——让 nav 直接 import registry 会把图标与所有工具 chunk 拉进首屏链；固定行 `tools/navigation.ts` + `appearance.navItems` 里的 `tool:<id>`（core 侧 `NAV_TOOL_IDS` 白名单校验）+ `nav.ts` 的 id 类型；页面 `ToolboxView.svelte`（**主界面与工具子页各一个 ⋯ 菜单**：`ContextMenu` 点锚定 + `UI颜色`/`背景颜色` + 传输页的 relay 子菜单；右键固定 / 拖动排序）；头部图标一律 `PAGE_HEADER_ICON_SIZE` | `references/ui-patterns.md`（工具箱）+ `references/history/v0.8.3.md` 三 + `references/history/v0.8.7.md` 七/八 |
