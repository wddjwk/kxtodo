...
- 定时配置按「时间 → 动作 → 结束」三个编号步骤排列，一次只展开当前步骤。顶部活句子可点击跳步、显示下次触发；日历支持每日/每周/每月与高级 Cron。参数、工作目录、超时和通知在折叠区；底部名称、启用、保存与试运行。试运行执行已保存配置，不消耗次数或改变排程；有未保存修改时先保存。
- 时间型触发可加窗口与探针门控，不匹配跳过这一轮且不算失败/次数；条件触发可选命中停用或冷却再检查；until 包含当天，日历触发用所选时区。移动端只给通知与无探针时间门控。运行记录复用原有 schedule.logs / JSONL，不展示未命中探针噪声。
- 原生网页预览使用独立桌面 Webview 或 Android Custom Tabs，开发浏览器保留 iframe。远程窗口不获 IPC，不访问本地 asset 协议，不导航主窗口；隐藏元数据窗口串行、有超时且禁止弹窗/下载。
- 卡片右键有「置顶 / 取消置顶」，搜索菜单也可操作。顶部置顶组沿用页面排序；设置「置顶条目样式」可独立切图标与可折叠分区，空分区隐藏，图标仅在折叠态显示。默认图标开、分区关；关闭样式不清除置顶数据。
- 列表排序保存到 `appearance.listSortMode`，并参与设置同步，不再在重启时回默认。
- 日记与记账的 FAB 容器横向居中对齐，「今」仍为小圆按钮，加号位置不变。
- 统计占比环按 `id:cents` 序列与总额的签名重新挂载：数据变化重播入场，纯聚焦与无数据变化的刷新不重播；这取代早期「数据变化不重播」口径。
- 真机验收待办（由维护者验收）：Windows/Linux 原生链接窗口复用、页面弹窗转浏览器、真实站点元数据与超时回收；Android Custom Tabs 返回、无 provider 时降级、真实软键盘下取色盘确认按钮与触屏拖色；三端定时配置视觉及通知时序。CI 构建通过不代替这些交互验证。
...
- **relay 服务**：一级菜单项「relay 服务 ›」+ 二级钻取（`MenuItem` 的 `submenu` 插槽），仅传输工具页渲染该菜单项；移动端自动钻入、桌面自动贴边翻转。（**v0.8.7 改**：二级里是「复用同步配置 / 使用默认服务 / 自定义服务」三选一，勾选在左、子菜单定宽、提示只留「下次上线时生效」——见下一节。）
...
### v0.8.7 的界面口径（改这些地方之前先读）

- **浮层定位只有两条分支**（`popover.ts::placePopover` 终裁；v0.8.6 的「下方限高 + 内部滚动」被真机打回、已删）：下方放得下 → 左上角顶点贴锚点；放不下 → 整个翻上、**下边缘贴锚点**；`mirrorXOnFlip`（`ContextMenu` 默认开）让右键/长按菜单翻上时横向镜像成右下角贴鼠标。四边一律钳制，唯一滚动兜底是「上下都放不下」的 maxHeight。**量高口径**：`Math.max(rect.height / scale, scrollHeight)`（`scrollHeight` 本就布局像素，不能再除）。
- **按钮锚定的菜单四件套**（新增菜单照抄，不要自创定位）：`x = rect.right` / `y = rect.bottom + 6` / `xAlign="right"` / `anchor={按钮}`——菜单贴按钮**正下方、右缘对齐按钮右缘**；同一个按钮再点一次收起（`openAppearanceMenu` 里比对 anchor）。工具箱主界面与工具子页的 ⋯ 菜单、（我的一天那些页面的）列表菜单、日期浮层、取色盘都走这一套。
- **工具箱的外观分两层**：工具箱主界面（`toolbox.accent` / `toolbox.backgroundColor`）+ **每个工具子页自己的**（`toolbox.toolAccents` / `toolbox.toolBackgrounds`，键是 `random/rmb/scratchpad/transfer`）。工具没配过就跟主界面；「默认」按钮在主界面回应用默认、在工具子页删掉自己那一键。**主界面与工具子页各有一个 ⋯ 菜单**（都在头部右侧），菜单结构 = 配置（仅传输页的 relay）→ 分割线 → `UI颜色` → `背景颜色`。
- **取色盘**：面板操作行多一枚「吸管」（只有 `'EyeDropper' in window` 才渲染）；面板外壳内是「反缩放层 + 色盘本体」两层（缩放壳里让 iro 的视觉尺寸 == 布局尺寸，见 `history/v0.8.7.md` 二.2）；**确认前会冲刷没跑的预览**（改了 HEX/RGB 直接点确认也生效）。宿主菜单里的「保存/取消」行（`ColorDraftActions`）已全部删除——**落盘按钮只在色盘面板上**。
- **预设色块编辑器**：右键预设色块弹出，**没有保存/取消按钮**——名称停输 300ms 自动落盘（失焦与菜单关闭再兜底提一次）、颜色框合法 HEX 即写并应用为当前背景；从编辑器色块打开的色盘，拖动 = 编辑器草稿 + 背景预览双写，确认写 `themePresets`、取消还原到**已存值**。点任意预设色块 = 选它作背景并结束编辑。
- **页面头部图标**：`styles.ts::PAGE_HEADER_ICON_SIZE`（34）是唯一口径（工具箱 / 工具子页 / 我的一天 / 日记 / 记账），`.toolbox-header-icon` 的盒子与 `.header-icon` 同为 38×38。
- **菜单里的勾选**：`MenuItem` 的 `checkable` 让勾选住在**最左侧的定宽槽位**（未选中留空格）——菜单宽度不随选中项变化。挂给排序方式 / 移动到分组 / 卡片类型 / 改账户 / 计划内分组 / relay 三选一。
- **relay 服务（三选一）**：复用同步配置（同步未配过时灰掉）/ 使用默认服务（出厂默认，= n0 公共 relay）/ 自定义服务；勾选在左、子菜单定宽（选项与输入框同宽）、提示只留「下次上线时生效」。
- **时刻双轨**：`.time-picker` 定宽 `calc(var(--font-control) * 6)`；`.time-col` 有 `overscroll-behavior: contain`（否则滑到顶/底会把下拉同步带出来）。灰带 `.time-band` 挂在 picker 里自动对齐，不用单独动。
