...
**v0.8.1 的两条渲染纪律**：① 卡片的完整渲染交给 `deferredMarkdown.ts`（双 rAF），让帧期间画折叠态内容，别把重活压在展开那一次 flush 里；② `setConfig` 一律**先本地生效再落盘**（等 IPC + 原子写回来才翻 UI = 点一下顿一下），失败按原值回滚。
