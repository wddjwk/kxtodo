...
- 写：`ledger add --amount <元> --account <账户名|ID> [--kind expense|income] [--category <分类名|ID>] [--date <YYYY-MM-DD>] [--time <HH:MM>] [--note <备注>] [--image <文件名>] --yes`。`--amount` 是元（两位小数，第三位四舍五入到分）；`--account`/`--category` **认名字也认 ID**（名字对人类与 Agent 更友好）。`--date` 缺省为本地今天。`--image` 是 `img/data/ledger/` 下的附图裸文件名（可选；不含路径分隔符）。返回体带 `accountName`/`categoryName`/`categoryParentName`/`signed`，不必二次查表。
...
- 改：`ledger modify --id ... --amount/--account/--category/--date/--time/--note/--image --yes`，字段缺省 = 不变（`--image ""` 清除附图）；**改转账也是 `modify`**（`--kind transfer --to <转入账户>`），不会另外新增一笔。删：`ledger remove --id ... --yes`（写同步墓碑）。
- 账户管理：`ledger account-add --name <名> [--kind <类型>] [--initial <元>] [--icon <lucide 名>] --yes`、`account-modify --id ... --yes`、`account-remove --id --yes`。`--kind` 是**自由字符串**（现金/银行卡/公积金/医保…任意非空值；预置清单见 `icon-list` 的 `accountGroups` 与界面），唯一有语义的是 `credit`（信用卡）——负余额计入总负债。**名下还有账目的账户删不掉**（`LEDGER_ACCOUNT_IN_USE`）——先改账或删账，别绕。
