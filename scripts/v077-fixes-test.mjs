...
// ---------------------------------------------------------------- 桌面
{
  ...
  await page.waitForSelector(".editor-tag-pop .tag-editor-input-row input:not([type=checkbox])", { timeout: 5000 });
  await page.fill(".editor-tag-pop .tag-editor-input-row input:not([type=checkbox])", "标签甲");
  ...
  await page.locator(".editor-meta-trigger.editor-tag-add").click();
  await page.waitForSelector(".editor-tag-pop .tag-editor-input-row input:not([type=checkbox])", { timeout: 5000 });
  // v0.8.3：标签浮层是右键菜单那一套，回车即添加（没有加号按钮了）
  await page.fill(".editor-tag-pop .tag-editor-input-row input:not([type=checkbox])", "标签乙");
